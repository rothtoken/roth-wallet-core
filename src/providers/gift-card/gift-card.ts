...
export class GiftCardProvider extends InvoiceProvider {
  ...
  listenForAuthChanges() {
    this.events.subscribe('rothId/Connected', async () => {
      await this.persistenceProvider.setrothIdSettings(this.getNetwork(), {
    ...
    this.events.subscribe('rothId/SettingsChanged', async () => {
      await this.getSupportedCardConfigMap(true);
    });
  }
  ...
  public async shouldSyncGiftCardPurchasesWithrothId() {
    const [user, userSettings] = await Promise.all([
      this.persistenceProvider.getrothIdUserInfo(this.getNetwork()),
      this.persistenceProvider.getrothIdSettings(this.getNetwork())
    ]);
    return user && userSettings && userSettings.syncGiftCardPurchases;
  }

  public getUserEmail(): Promise<string> {
    const getrothIdInfo = this.persistenceProvider.getrothIdUserInfo(
      this.getNetwork()
    );
    const getGiftCardUserInfo = this.persistenceProvider.getGiftCardUserInfo();
    const shouldSync = this.shouldSyncGiftCardPurchasesWithrothId();
    return Promise.all([shouldSync, getrothIdInfo, getGiftCardUserInfo])
      .then(([shouldSync, ...rest]) => {
        const [rothIdEmail, giftCardEmail] = rest
          .map(result => (_.isString(result) ? JSON.parse(result) : result))
          .map(jsonResult => jsonResult && jsonResult.email);
        return (
          (shouldSync && rothIdEmail) ||
          giftCardEmail ||
  ...
  public async createrothInvoice(data, attempt: number = 1) {
    this.logger.info('roth Creating Invoice: try... ' + attempt);
    const params = {
      brand: data.cardName,
    ...
    const shouldSync = await this.shouldSyncGiftCardPurchasesWithrothId();
    const promise = shouldSync
      ? this.createAuthenticatedrothInvoice.bind(this)
      : this.createUnauthenticatedrothInvoice.bind(this);

    const cardOrder = await promise(params).catch(async err => {
      this.logger.error('roth Create Invoice: ERROR', JSON.stringify(err));
      if (attempt <= 5 && err.status == 403) {
        await new Promise(resolve => setTimeout(resolve, 3000 * attempt));
        return this.createrothInvoice(data, ++attempt);
      } else throw err;
    });
    this.logger.info('roth Create Invoice: SUCCESS');
    return cardOrder as {
      accessKey: string;
      invoiceId: string;
  ...
  public async createUnauthenticatedrothInvoice(params) {
  ...
  public async createAuthenticatedrothInvoice(params) {
    const user = await this.persistenceProvider.getrothIdUserInfo(
      this.getNetwork()
    );
    return this.rothIdProvider.apiCall('createGiftCardInvoice', {
      ...params,
      email: user.email
    });
  }
    ...
    return from(cardsNeedingUpdate).pipe(
      ...
      switchMap(card =>
        card.status === 'UNREDEEMED' || card.status === 'PENDING'
          ? fromPromise(
              this.getrothInvoice(card.invoiceId).then(invoice => ({
                ...card,
                status:
                  (card.status === 'PENDING' ||
  ...
  async fetchAvailableCardMap() {
    const shouldSync = await this.shouldSyncGiftCardPurchasesWithrothId();
    const userInfo = await this.persistenceProvider.getrothIdUserInfo(
      this.getNetwork()
    );
...
function appendFallbackImages(cardConfig: CardConfig) {
  ...
  const getBrandImagePath = brandName => {
    const cardImagePath = `https://roth.com/gift-cards/assets/`;
  ...
  const needsFallback =
    cardConfig &&
    cardConfig.cardImage &&
    !cardConfig.cardImage.includes('https://roth.com');
