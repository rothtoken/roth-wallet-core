...
const Keys = {
  ...
  roth_ACCOUNTS_V2: network => 'rothAccounts-v2-' + network,
  ...
  roth_ID_PAIRING_TOKEN: network => `rothIdToken-${network}`,
  roth_ID_USER_INFO: network => `rothIdUserInfo-${network}`,
  roth_ID_SETTINGS: network => `rothIdSettings-${network}`,
  APP_THEME: 'app-theme',
...
export class PersistenceProvider {
  ...
  getrothAccounts(network: string) {
    return this.storage.get(Keys.roth_ACCOUNTS_V2(network));
  }
  ...
  setrothAccount(
    ...
    return this.getrothAccounts(network).then(allAccounts => {
      ...
      this.logger.info(
        'Storing roth accounts with new account:' + data.email
      );
      return this.storage.set(Keys.roth_ACCOUNTS_V2(network), allAccounts);
    });
  }

  removerothAccount(network: string, email: string) {
    return this.getrothAccounts(network).then(allAccounts => {
      allAccounts = allAccounts || {};
  ...
  removerothAccountV2(network: string) {
    return this.storage.set(Keys.roth_ACCOUNTS_V2(network), {});
  }

  setrothDebitCards(network: string, email: string, cards) {
    return this.storage.set(Keys.roth_ACCOUNTS_V2(network), {
      [email]: { cards }
    });
  }
  ...
  //   email: account email
  // ]
  getrothDebitCards(network: string) {
    return this.getrothAccounts(network).then(allAccounts => {
      let allCards = [];
  ...
  removerothDebitCard(network: string, cardEid: string) {
    return this.getrothAccounts(network)
      .then(allAccounts => {
  ...
  removeAllrothAccounts(network: string) {
    return this.storage.set(Keys.roth_ACCOUNTS_V2(network), {});
  }
  ...
  setrothIdPairingToken(network: Network, token: string) {
    return this.storage.set(Keys.roth_ID_PAIRING_TOKEN(network), token);
  }

  getrothIdPairingToken(network: Network) {
    return this.storage.get(Keys.roth_ID_PAIRING_TOKEN(network));
  }

  removerothIdPairingToken(network: Network) {
    return this.storage.remove(Keys.roth_ID_PAIRING_TOKEN(network));
  }

  setrothIdUserInfo(network: Network, userInfo: any) {
    return this.storage.set(Keys.roth_ID_USER_INFO(network), userInfo);
  }

  getrothIdUserInfo(network: Network) {
    return this.storage.get(Keys.roth_ID_USER_INFO(network));
  }

  removerothIdUserInfo(network: Network) {
    return this.storage.remove(Keys.roth_ID_USER_INFO(network));
  }

  setrothIdSettings(network: Network, userSettings: any) {
    return this.storage.set(Keys.roth_ID_SETTINGS(network), userSettings);
  }

  getrothIdSettings(network: Network) {
    return this.storage.get(Keys.roth_ID_SETTINGS(network));
  }
  ...
  removerothIdSettings(network: Network) {
    return this.storage.remove(Keys.roth_ID_SETTINGS(network));
  }
  setrothIdPairingFlag(value: string) {
    this.logger.debug('card experiment enabled: ', value);
    return this.storage.set('rothIdPairingFlag', value);
  }

  getrothIdPairingFlag() {
    return this.storage.get('rothIdPairingFlag');
  }

  removerothIdPairingFlag() {
    return this.storage.remove('rothIdPairingFlag');
  }
  ...
  setDynamicLink(deepLink: string) {
    return this.storage.set('roth-DynamicLink', deepLink);
  }

  getDynamicLink() {
    return this.storage.get('roth-DynamicLink');
  }

  removeDynamicLink() {
    return this.storage.remove('roth-DynamicLink');
  }
