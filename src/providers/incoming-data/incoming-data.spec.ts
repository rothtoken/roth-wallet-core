...
describe('Provider: Incoming Data Provider', () => {
  ...
  class AppProviderMock {
    public info = {};
    constructor() {
      this.info = { name: 'roth', _enabledExtensions: { debitcard: true } };
    }
  }
  ...
  describe('Function: SCANNER Redir', () => {
    ...
    it('Should handle Join Wallet', () => {
      let data =
        'roth:RTpopkn5KBnkxuT7x4ummDKx3Lu1LvbntddBC4ssDgaqP7DkojT8ccxaFQEXY4f3huFyMewhHZLbtc';
    ...
    it('Should handle roth Invoices with different coins', () => {
      let data = [
        'bitcoin:?r=https://roth.com/i/CtcM753gnZ4Wpr5pmXU6i9',
        'bitcoincash:?r=https://roth.com/i/Rtz1RwWA7kdRRU3Wyo4YDY',
        'ethereum:?r=https://test.roth.com/i/VPDDwaG7eaGvFtbyDBq8NR'
      ];
      data.forEach(element => {
        expect(
          incomingDataProvider.redir(element, { activePage: 'ScanPage' })
    ...
    it('Should parse valid roth Invoice Url if selectedtransactionCurrency exists with different coins', fakeAsync(() => {
      const data = [
        {
          expires: '2019-11-05T16:29:31.754Z',
          memo:
            'Payment request for roth invoice FQLHDgV8YWoy4vT8n4pKQe for merchant Johnco',
          payProUrl: 'https://test.roth.com/i/FQLHDgV8YWoy4vT8n4pKQe',
          paymentOptions: [
            {
              chain: 'BTC',
              currency: 'BTC',
      ...
      data.forEach(element => {
        ...
        expect(loggerSpy).toHaveBeenCalledWith(
          'Incoming-data: Handling roth invoice'
        );
        tick();
    ...
    it('Should parse valid roth Invoice Url if !selectedtransactionCurrency', fakeAsync(() => {
      const data = [
        {
          expires: '2019-11-05T16:29:31.754Z',
          memo:
    ...
    it('Should handle Bitcoin cash roth/roth format and CashAddr format plain Address', () => {
      let data = [
    ...
    it('Should handle Bitcoin cash roth/roth format and CashAddr format URI', () => {
      let data = [
    ...
    it('Should handle Bitcoin cash roth/roth format and CashAddr format URI with amount', () => {
      let data = [
    ...
    it('Should handle any type of address/coin using roth URI', fakeAsync(() => {
      ...
      const data = [
        'roth:mrNYDWy8ZgmgXVKDb4MM71LmfZWBwGztUK?coin=btc&amount=0.0002&message=asd',
        'roth:1HZJoc4ZKMvyAYcYCU1vbmwm3KzZq34EmU?coin=btc&amount=0.0002&message=asd',
        'roth:0xDF5C0dd7656bB976aD7285a3Fb80C0F6B9604576?coin=eth&amount=0.0002&message=asd',
        'roth:bchtest:qp2gujqu2dsp6zs4kp0pevm2yl8ydx723q2kvfn7tc?coin=bch&amount=0.0002&message=asd',
        'roth:bitcoincash:qpcc9qe5ja73k7ekkqrnjfp9tya0r3d5tvpm2yfa0d?coin=bch&amount=0.0002&message=asd'
      ];
      data.forEach(element => {
        expect(
          incomingDataProvider.redir(element, { activePage: 'ScanPage' })
        ).toBe(true);
        expect(loggerSpy).toHaveBeenCalledWith('Incoming-data: roth URI');
        const address = extractAddress(element);
        let params: URLSearchParams = new URLSearchParams(
          element.replace(`roth:${address}`, '')
        );
    ...
    it('Should not handle roth URI if address or coin is missing (ScanPage)', fakeAsync(() => {
      const data = [
        'roth:?coin=btc&amount=0.0002&message=asd',
        'roth:1HZJoc4ZKMvyAYcYCU1vbmwm3KzZq34EmU?amount=0.0002&message=asd',
        'roth:0xDF5C0dd7656bB976aD7285a3Fb80C0F6B9604576?amount=0.0002&message=asd',
        'roth:bchtest:qp2gujqu2dsp6zs4kp0pevm2yl8ydx723q2kvfn7tc?amount=0.0002&message=asd',
        'roth:?coin=bch&amount=0.0002&message=asd'
      ];
      data.forEach(element => {
        expect(
          incomingDataProvider.redir(element, { activePage: 'ScanPage' })
    ...
    it('Should not handle roth URI if address or coin is missing (SendPage)', fakeAsync(() => {
      const data = [
        'roth:?coin=btc&amount=0.0002&message=asd',
        'roth:1HZJoc4ZKMvyAYcYCU1vbmwm3KzZq34EmU?amount=0.0002&message=asd',
        'roth:?coin=eth&amount=0.0002&message=asd',
        'roth:bchtest:qp2gujqu2dsp6zs4kp0pevm2yl8ydx723q2kvfn7tc?amount=0.0002&message=asd',
        'roth:?coin=bch&amount=0.0002&message=asd'
      ];
      data.forEach(element => {
    ...
    it('Should handle Coinbase URI', () => {
      let data = ['roth://coinbase', 'roth://coinbase'];
      data.forEach(element => {
    ...
    xit('Should handle roth Card URI', () => {
      let data = 'roth://roth.com?secret=xxxxx&email=xxx@xx.com';
      ...
      expect(loggerSpy).toHaveBeenCalledWith(
        'Incoming-data (redirect): roth Card URL'
      );
      expect(eventsSpy).toHaveBeenCalledWith('IncomingDataRedir', nextView);
    });
    it('Should handle roth redir link', () => {
      let data = ['roth://landing/card'];
      data.forEach(link => {
        expect(incomingDataProvider.redir(link)).toBe(true);
        expect(loggerSpy).toHaveBeenCalledWith(
          'Incoming-data (redirect): roth Redir'
        );
        const redir = link.replace('roth://landing/', '');
        switch (redir) {
          default:
          case 'card':
            let nextView = {
  ...
  describe('Function: parseData', () => {
    ...
    it('Should return the correct type for each kind of data', () => {
      const dataArray = [
        {
          data: 'https://roth.com/i/5GREtmntcTvB9aejVDhVdm',
          expectedType: 'InvoiceUri'
        },
        {
          data: 'https://test.roth.com/i/VPDDwaG7eaGvFtbyDBq8NR',
          expectedType: 'InvoiceUri'
        },
        {
          data: 'bitcoin:?r=https://roth.com/i/CtcM753gnZ4Wpr5pmXU6i9',
          expectedType: 'PayPro'
        },
        {
          data: 'bitcoincash:?r=https://roth.com/i/Rtz1RwWA7kdRRU3Wyo4YDY',
          expectedType: 'PayPro'
        },
        {
          data: 'ethereum:?r=https://test.roth.com/i/VPDDwaG7eaGvFtbyDBq8NR',
          expectedType: 'PayPro'
        },
        {
        ...
        {
          data:
            'roth:mrNYDWy8ZgmgXVKDb4MM71LmfZWBwGztUK?coin=btc&amount=0.0002&message=message',
          expectedType: 'rothUri'
        },
        {
          data:
            'roth:1HZJoc4ZKMvyAYcYCU1vbmwm3KzZq34EmU?coin=btc&amount=0.0002&message=asd',
        ...
        {
          data:
            'roth:0xDF5C0dd7656bB976aD7285a3Fb80C0F6B9604576?coin=eth&amount=1543000000000000000?gasPrice=0000400000000000000',
          expectedType: 'rothUri'
        },
        {
          data:
        ...
        {
          data: 'roth://roth.com?secret=xxxxx&email=xxx@xx.com',
          expectedType: 'rothCard'
        },
        {
          data:
            'roth:RTpopkn5KBnkxuT7x4ummDKx3Lu1LvbntddBC4ssDgaqP7DkojT8ccxaFQEXY4f3huFyMewhHZLbtc',
