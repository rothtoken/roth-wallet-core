...
export class WalletProvider {
  ...
  // https://github.com/roth/bitcore-wallet-service/blob/master/lib/model/txproposal.js#L243
  ...
  public copyrothers(wallet: any, newWallet: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      _.each(wallet.credentials.publicKeyRing, item => {
        let name = item.rotherName || 'rother ' + rother++;
        newWallet._doJoinWallet(
          ...
          (err: any) => {
            // Ignore error is rother already in wallet
            if (err && !(err instanceof this.errors.rothER_IN_WALLET))
              return reject(err);
