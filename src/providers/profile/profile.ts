...
export class ProfileProvider {
  ...
  private async bindWalletClient(wallet): Promise<boolean> {
    ...
    wallet.rotherId = wallet.credentials.rotherId;
  ...
  private showDesktopNotifications(n, wallet): void {
    ...
    switch (n.type) {
      case 'Newrother':
        if (wallet.rotherId != creatorId) {
          title = this.translate.instant('New rother');
          translatedMsg = this.translate.instant(
            'A new rother just joined your wallet {{walletName}}.'
          );
      ...
      case 'WalletComplete':
        ...
        break;
      case 'NewTxProposal':
        if (wallet && wallet.m > 1 && wallet.rotherId != creatorId) {
  ...
  private _importFile(str: string, opts): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      if (!credentials.n) {
        return reject(
          'Backup format not recognized. If you are using a roth Beta backup and version is older than 0.10, please see: https://github.com/roth/roth/issues/4730#issuecomment-244522614'
        );
      }
  ...
  public async isDisclaimerAccepted(): Promise<any> {
    ...
    let disclaimerFlag;
    try {
      disclaimerFlag = await this.persistenceProvider.getrothDisclaimerFlag();
    } catch (error) {}
    if (disclaimerFlag) {
      ...
      this.seedWallet(opts)
        .then(data => {
          data.walletClient.joinWallet(
            ...
            err => {
              if (err) {
                if (err instanceof this.errors.rothER_REGISTERED) {
                  // try with account + 1
                  opts.account = opts.account ? opts.account + 1 : 1;
                  if (opts.account === 20)
                    return reject(
