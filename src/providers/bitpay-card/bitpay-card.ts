...
export class rothCardProvider {
  constructor(
    ...
    this.logger.debug('rothCardProvider initialized');
  }
  ...
  public sync(apiContext, cb) {
    ...
    this.onGoingProcessProvider.set('fetchingrothCards');
    // Get Debit Cards
    this.rothProvider.post(
      ...
      data => {
        ...
        this.logger.info('roth Get Debit Cards: SUCCESS');
        ...
        _.each(data.data, x => {
          ...
          if (!x.eid || !x.id || !x.lastFourDigits || !x.token) {
            this.logger.warn('BAD data from roth card' + JSON.stringify(x));
            return;
          }
      ...
      data => {
        this.onGoingProcessProvider.clear();
        return cb(this._setError('roth Card Error: Get Debit Cards', data));
      }
    );
  }
      ...
      err => {
        ...
        this.getCards(data => {
          ...
          this.rothProvider.post(
            ...
            json,
            data => {
              this.logger.info('roth Get Invoices: SUCCESS');
              invoices = data.data || [];
              ...
              this.rothProvider.post(
                ...
                json,
                data => {
                  this.logger.info('roth Get History: SUCCESS');
                  history = data.data || {};
                ...
                data => {
                  this.logger.info(
                    'Error loading roth Card transaction history for ',
                    'card id: ',
                    cardId,
                    'Message: ',
                    data.error
                  ...
                  return cb(
                    this._setError('roth Card Error: Get History', data)
                  );
                }
              );
            },
            data => {
              this.logger.info(
                'Error loading roth Card invoice history for ',
                'card id: ',
                cardId,
                'Message: ',
                data.error
              ...
              return cb(
                this._setError('roth Card Error: Get Invoices', data)
              );
            }
          );
        });
      ...
      err => {
        ...
        this.getCards(data => {
          ...
          this.rothProvider.post(
            ...
            res => {
              if (res.error) {
                this.logger.error('roth TopUp: With Errors');
                return cb(res.error);
              } else {
                this.logger.info('roth TopUp: SUCCESS');
                return cb(null, res.data);
              }
            },
            res => {
              return cb(this._setError('roth Card Error: TopUp', res));
            }
          );
        });
      }
  ...
  public getInvoice(id, cb) {
    this.rothProvider.get(
      ...
      res => {
        return cb(this._setError('roth Card Error: Get Invoice', res));
      }
    );
  }
  ...
  public getCards(cb) {
    this.persistenceProvider
      .getrothDebitCards(this.rothProvider.getEnvironment().network)
      .then(val => {
        return cb(val);
      });
  }
  ...
  public remove(cardId, cb) {
    this.persistenceProvider
      ...
      .catch(err => {
        this.logger.error('Error removing roth debit card: ' + err);
        return cb(err);
      });
  }
  ...
  public getRates(currency, cb) {
    this.rothProvider.get(
      '/rates/' + currency,
      data => {
        this.logger.info('roth Get Rates: SUCCESS');
        return cb(data.error, data.data);
      },
      data => {
        return cb(this._setError('roth Error: Get Rates', data));
      }
    );
  }
  ...
  public register() {
    this.isActive(isActive => {
      this.homeIntegrationsProvider.register({
        name: 'debitcard',
        title: 'roth Visa® Card',
        icon: 'assets/img/roth-card/icon-roth.svg',
        page: 'rothCardIntroPage',
