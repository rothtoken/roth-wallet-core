...
export class IABCardProvider {
  ...
  init(): void {
    ...
    this.cardIAB_Ref.events$.subscribe(async (event: any) => {
      ...
      switch (event.data.message) {
        ...
        /*
         *
         * This handles the roth ID pairing and retrieves user data. It also passes it to the behavior subject.
         *
         * */
  ...
  apiCall(json): any {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const url = `${this.roth_API_URL}/api/v2/graphql`;
  ...
  getCards() {
    return new Promise(async resolve => {
      ...
      if (data && data.user && data.user.cards) {
        ...
        let currentCards = await this.persistenceProvider.getrothDebitCards(
          Network[this.NETWORK]
        );
        ...
        await this.persistenceProvider.setrothDebitCards(
          Network[this.NETWORK],
          user.email,
          cards
        );
  ...
  getBalances() {
    return new Promise(async resolve => {
      ...
      if (data && data.user && data.user.cards) {
        ...
        const currentCards = await this.persistenceProvider.getrothDebitCards(
          Network[this.NETWORK]
        );
  ...
  async balanceUpdate(event) {
    let cards = await this.persistenceProvider.getrothDebitCards(
      Network[this.NETWORK]
    );
  ...
  async graphRequest(request): Promise<any> {
    try {
      ...
      const { priv, pub } = await this.rothIdProvider.getAppIdentity();

      let url = `https://roth.com/`;
      const dataToSign = `${url}${JSON.stringify(json)}`;
      const signedData = bitauthService.sign(dataToSign, priv);
  ...
  async pairing(event) {
    ...
    this.onGoingProcess.set('connectingrothId');

    await this.persistenceProvider.removeAllrothAccounts(
      Network[this.NETWORK]
    );

    // generates pairing token and also fetches user basic info and caches both
    this.rothIdProvider.generatePairingToken(
      params,
      async (user: User) => {
        if (user) {
          ...
          this.events.publish('rothId/Connected');

          // if with notification -> connect your roth id in settings or pairing from personal dashboard
          if (withNotification) {
            ...
            const infoSheet = this.actionSheetProvider.createInfoSheet(
              'in-app-notification',
              {
                ...
                body: this.translate.instant(
                  'roth ID successfully connected.'
                )
              }
            );
        ...
        const request = {
          query: `
            mutation START_CREATE_PROVISIONING_REQUEST($token:String!, $csrf:String, $cardId:String!, $input:ProvisionInputType!) {
              user:rothUser(token:$token, csrf:$csrf) {
