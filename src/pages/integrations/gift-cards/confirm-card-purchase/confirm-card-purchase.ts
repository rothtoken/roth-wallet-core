...
export class ConfirmCardPurchasePage extends ConfirmPage {
  ...
  public async createInvoice(data) {
    ...
    const invoice = await this.giftCardProvider
      .getrothInvoice(cardOrder.invoiceId)
      .catch(() => {
        throw {
  ...
  public async refreshCardConfig() {
    ...
    this.rothIdProvider.refreshUserInfo();
  ...
  public async refreshCardConfigIfNeeded() {
    const shouldSync = await this.giftCardProvider.shouldSyncGiftCardPurchasesWithrothId();
    if (shouldSync) {
