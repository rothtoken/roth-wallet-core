...
import { rothProvider } from '../../../../providers/roth/roth';
...
@Component({
  selector: 'page-roth-card-topup',
  templateUrl: 'roth-card-topup.html'
})
export class rothCardTopUpPage {
  ...
  ionViewDidLoad() {
    this.logger.info('Loaded: rothCardTopUpPage');
  }
  ...
  ionViewWillEnter() {
    ...
    this.rothCardProvider
      ...
      .then(card => {
        this.rothCardProvider.setCurrencySymbol(card[0]);
        ...
        this.brand === 'Mastercard'
          ? this.rothCardProvider.logEvent('mastercard_topup_amount', {
              usdAmount: this.amount,
              transactionCurrency: 'USD'
            })
          : this.rothCardProvider.logEvent('legacycard_topup_amount', {
              usdAmount: this.amount,
              transactionCurrency: 'USD'
            });

        const network = this.rothProvider.getEnvironment().network;
  ...
  private updateRates(coin: string) {
    this.rothCardProvider.getRatesFromCoin(
      coin.toUpperCase(),
  ...
  private createInvoice(data): Promise<any> {
    return new Promise((resolve, reject) => {
      this.rothCardProvider.topUp(this.cardId, data, (err, invoiceId) => {
        if (err) {
          return reject({
        ...
        this.rothCardProvider.getInvoice(invoiceId, (err, inv) => {
          if (err) {
            return reject({
  ...
  logCardTopUpEvent(coin, isConfirm) {
    ...
    !isConfirm
      ? this.rothCardProvider.logEvent(
  ...
  logCardPurchaseEvent() {
    this.brand === 'Mastercard'
      ? this.rothCardProvider.logEvent('purchase', {
          value: this.amount,
          items: [
            {
              name: 'mastercard',
  ...
  logCardAddToCartEvent() {
    ...
    this.rothCardProvider.logEvent('add_to_cart', {
      items: [
        {
          name: cardName,
          category: 'debitCard'
  ...
  logCardSetCheckoutOption(wallet) {
    this.brand === 'Mastercard'
      ? this.rothCardProvider.logEvent('mastercard_set_checkout_option', {
          checkout_option: wallet.coin,
          checkout_step: 1
        })
      : this.rothCardProvider.logEvent('legacycard_set_checkout_option', {
          checkout_option: wallet.coin,
          checkout_step: 1
        });
  }
  ...
  public openExternalLink(urlKey: string) {
    ...
    switch (urlKey) {
      case 'networkCost':
        url =
          'https://support.roth.com/hc/en-us/articles/115002990803-Why-Am-I-Being-Charged-an-Additional-Network-Cost-on-My-roth-Invoice-';
        title = this.translate.instant('Network Cost');
        break;
      case 'minerFee':
        url =
          'https://support.roth.com/hc/en-us/articles/115003393863-What-are-bitcoin-miner-fees-Why-are-miner-fees-so-high-';
