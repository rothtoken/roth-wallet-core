...
import { rothIdProvider, IABCardProvider } from '../../providers';
...
import { rothSettingsPage } from '../integrations/roth-card/roth-settings/roth-settings';
...
import { rothIdPage } from './roth-id/roth-id';
...
export class SettingsPage {
  ...
  public rothIdPairingEnabled: boolean;
  public rothIdUserInfo: any;
  private network = Network[this.rothIdProvider.getEnvironment().network];
  ...
  async ionViewWillEnter() {
    this.persistenceProvider
      ...
      .then(res => (this.rothIdPairingEnabled = res === 'enabled'));

    if (this.iabCardProvider.ref) {
      // check for user info
      this.persistenceProvider
        .getrothIdUserInfo(this.network)
        .then((user: User) => {
          this.rothIdUserInfo = user;
        });
  ...
  ionViewDidEnter() {
    ...
    // Only roth Wallet
    this.rothCardProvider.get({ noHistory: true }).then(cards => {
      this.showrothCard = !!this.app.info._enabledExtensions.debitcard;
      this.rothCardItems = cards;
    });
  }
  ...
  public openrothIdPage(): void {
    if (this.rothIdUserInfo) {
      this.navCtrl.push(rothIdPage, this.rothIdUserInfo);
    } else {
  ...
  public openSettingIntegration(name: string): void {
    switch (name) {
      case 'coinbase':
        ...
        break;
      case 'debitcard':
        this.navCtrl.push(rothSettingsPage);
        break;
      case 'buycrypto':
  ...
  public openHelpExternalLink(): void {
    ...
    const url =
      this.appName == 'roth'
        ? 'https://github.com/roth/roth/issues'
        : 'https://help.roth.com/roth-app';
    const optIn = true;
    const title = null;
  ...
  public openSupportEncryptPassword(): void {
    const url =
      'https://support.roth.com/hc/en-us/articles/360000244506-What-Does-a-Spending-Password-Do-';
    const optIn = true;
    const title = null;
  ...
  public openPrivacyPolicy() {
    const url = 'https://roth.com/about/privacy';
    const optIn = true;
    const title = null;
  ...
  public openTermsOfUse() {
    const url = 'https://roth.com/legal/terms-of-use/#wallet-terms-of-use';
    const optIn = true;
    const title = null;
