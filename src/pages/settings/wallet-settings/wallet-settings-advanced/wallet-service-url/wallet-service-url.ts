...
export class WalletServiceUrlPage {
  ...
  ionViewWillEnter() {
    ...
    this.comment = this.replaceParametersProvider.replace(
      this.translate.instant(
        "{{appName}} depends on Bitcore Wallet Service (BWS) for blockchain information, networking and rother synchronization. The default configuration points to https://bws.roth.com (roth's public BWS instance)."
      ),
      { appName }
    );
  ...
  public save(): void {
    ...
    switch (this.walletServiceForm.value.bwsurl) {
      case 'prod':
      case 'production':
        bws = 'https://bws.roth.com/bws/api';
        break;
      case 'sta':
      case 'staging':
