...
export class WalletDuplicatePage {
  ...
  public duplicate(wallet) {
    ...
    let opts: any = {
      ...
      m: wallet.m,
      n: wallet.n,
      myName: wallet.credentials.rotherName,
      ...
      .then(key => {
        ...
        this.importOrCreate(wallet, opts)
          .then(result => {
            ...
            // Multisig wallets? add rothers
            this.addrothers(wallet, newWallet, isNew)
              .then(() => {
  ...
  private addrothers(wallet, newWallet, isNew): Promise<any> {
    if (!isNew) return Promise.resolve();
    if (wallet.n == 1) return Promise.resolve();

    this.logger.debug(
      `Adding rothers for BCH wallet config: ${wallet.m} - ${wallet.n}`
    );

    return this.walletProvider.copyrothers(wallet, newWallet);
  }
  ...
  public openHelpExternalLink(): void {
    let url =
      'https://support.roth.com/hc/en-us/articles/115005019583-How-Can-I-Recover-Bitcoin-Cash-BCH-from-My-Wallet-';
    let optIn = true;
    let title = null;
