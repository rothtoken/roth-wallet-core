...
import { rothCardTopUpPage } from '../../integrations/roth-card/roth-card-topup/roth-card-topup';
...
export class AmountPage {
  ...
  constructor(
    ...
    // roth Card ID or Wallet ID or Coinbase Account ID
    ...
    if (_.isNumber(result)) {
      ...
      if (this.availableUnits[this.unitIndex].isFiat) {
        ...
        if (a) {
          ...
          this.checkAmountForrothcard(result);
        } else {
        ...
        this.checkAmountForrothcard(this.toFiat(result));
      }
    }
  }

  private checkAmountForrothcard(amount: number): void {
    // Check if the top up amount is at least 1 usd
    const isTopUp =
      this.navParams.data.nextPage === 'rothCardTopUpPage' ? true : false;
    if (isTopUp && amount < 1) {
      this.allowSend = false;
    }
  }
