...
@Component({
  selector: 'page-rothers',
  templateUrl: 'rothers.html'
})
export class rothersPage {
  ...
  ionViewDidLoad() {
    this.logger.info('Loaded: rothersPage');
  }
  ...
  private walletUpdate(opts): void {
    ...
    if (this.wallet && opts.walletId == this.wallet.id) {
      ...
      if (
        ...
        !rothersPage.processed[opts.walletId]
      ) {
        rothersPage.processed[opts.walletId] = true;
        // TODO?
