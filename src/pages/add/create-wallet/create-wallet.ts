...
// Pages
import { rothersPage } from '../../add/rothers/rothers';
...
export class CreateWalletPage implements OnInit {
  ...
  constructor(
    ...
    this.tc = this.isShared ? this.defaults.wallet.totalrothers : 1;
    this.rothers = _.range(2, this.defaults.limits.totalrothers + 1);
  ...
  public setTotalrothers(n: number): void {
    this.createForm.controls['totalrothers'].setValue(n);
  }

  private updateRCSelect(n: number): void {
    const maxReq = this.rothER_PAIR_LIMITS[n];
    this.signatures = _.range(1, maxReq + 1);
    this.createForm.controls['requiredrothers'].setValue(
  ...
  public setOptsAndCreate(): void {
    const opts: Partial<WalletOptions> = {
      ...
      m: this.createForm.value.requiredrothers,
      n: this.createForm.value.totalrothers,
      myName:
        this.createForm.value.totalrothers > 1
        ...
        this.navCtrl.popToRoot().then(() => {
          ...
          setTimeout(() => {
            if (wallet.isComplete()) {
              ...
              const rotherModal = this.modalCtrl.create(
                rothersPage,
                {
                  walletId: wallet.credentials.walletId
                },
  ...
  public openSupportSingleAddress(): void {
    const url =
      'https://support.roth.com/hc/en-us/articles/360015920572-Setting-up-the-Single-Address-Feature-for-your-roth-Wallet';
    const optIn = true;
    const title = null;
  ...
  public set(type: string, number: number) {
    switch (type) {
      case 'requiredrothers':
        if (this.signatures.includes(number)) {
          this.createForm.controls['requiredrothers'].setValue(number);
        }
        break;

      case 'totalrothers':
        if (this.rothers.includes(number)) {
          this.createForm.controls['totalrothers'].setValue(number);
        }
        break;
    }
  }
