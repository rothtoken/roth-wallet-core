  ...
  public async sendFeedback(feedback: string, goHome: boolean): Promise<void> {
    ...
    if (this.platformProvider.isElectron) {
      version = this.platformProvider
        .getDeviceInfo()
        .match(/(Electron[\/]\d+(\.\d)*)/i)[0]; // getDeviceInfo example: 5.0 (Macintosh; Intel Mac OS X 10_14_1) AppleWebKit/537.36 (KHTML, like Gecko) roth/5.1.0 Chrome/66.0.3359.181 Electron/3.0.8 Safari/537.36
      platform =
  ...
  public showInfoSheet(key: InfoSheetType, externalLink: string): void {
    ...
    infoSheet.onDidDismiss(async option => {
      if (!option) {
        ...
        // Click on Get Help
        this.openExternalLink('https://support.roth.com/hc/en-us');
      }
    });
  }
}
