...
export class TxpDetailsPage {
  ...
  constructor(
    ...
    this.rothers = this.wallet.cachedStatus.wallet.rothers;
    this.rotherId = this.wallet.credentials.rotherId;
    ...
    this.walletProvider
      ...
      .then(tx => {
        let action: any = _.find(tx.actions, {
          rotherId: this.wallet.credentials.rotherId
        });
  ...
  public updaterotherList(): void {
    _.map(this.rothers, (cp: any) => {
      _.each(this.tx.actions, ac => {
        if (cp.id == ac.rotherId) {
          cp.action = ac.type;
        }
      });
    });
