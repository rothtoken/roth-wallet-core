...
import { rothCardIntroPage } from '../integrations/roth-card/roth-card-intro/roth-card-intro';
import { PhaseOneCardIntro } from '../integrations/roth-card/roth-card-phases/phase-one/phase-one-intro-page/phase-one-intro-page';
...
export class HomePage {
    ...
    this.persistenceProvider.getDynamicLink().then((deepLink: string) => {
      if (deepLink) {
        ...
        this.persistenceProvider
          ...
          .then((value: string) => {
            if (value === 'enabled' && this.appProvider.info.name !== 'roth')
              this.openAddFunds();
          });
      }
    });
  ...
  private setMerchantDirectoryAdvertisement() {
    ...
    !alreadyVisible &&
      this.advertisements.push({
        ...
        link: 'https://roth.com/directory/?hideGiftCards=true',
    ...
    this.events.subscribe(
      ...
      ({ status, cards, cardExperimentEnabled }) => {
        ...
        switch (status) {
          case 'connected':
            hasGalileo
              ? this.removeAdvertisement('roth-card')
              : this.addrothCard();
            break;
          case 'disconnected':
            this.addrothCard();
          ...
          default:
            ...
            if (!hasGalileo) this.addrothCard();
        }
      }
    );
  ...
  private addrothCard() {
    ...
    this.persistenceProvider
      .getAdvertisementDismissed('roth-card')
      .then((value: string) => {
        if (value === 'dismissed') {
          return;
        }
        const card: Advertisement = this.cardExperimentEnabled
          ? {
              name: 'roth-card',
              title: this.translate.instant('Get the roth Card'),
              body: this.translate.instant(
                'Designed for people who want to live life on crypto.'
              ),
              app: 'roth',
              linkText: this.translate.instant('Order Now'),
              link: rothCardIntroPage,
              isTesting: false,
              dismissible: true,
              imgSrc: 'assets/img/roth-card/roth-card-mc-angled-plain.svg'
            }
          : {
              name: 'roth-card',
  ...
  public openCountryBannedLink(): void {
    const url =
      "https://github.com/roth/roth/wiki/Why-can't-I-use-roth's-services-in-my-country%3F";
    this.externalLinkProvider.open(url);
  }
  ...
  private openEmailDisclaimer() {
    const message = this.translate.instant(
      'By providing your email address, you give explicit consent to roth to use your email address to send you email notifications about payments.'
    );
