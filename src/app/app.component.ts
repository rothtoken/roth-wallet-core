...
import { rothersPage } from '../pages/add/rothers/rothers';
import { ImportWalletPage } from '../pages/add/import-wallet/import-wallet';
import { JoinWalletPage } from '../pages/add/join-wallet/join-wallet';
import { ExchangeCryptoPage } from '../pages/exchange-crypto/exchange-crypto';
import { FingerprintModalPage } from '../pages/fingerprint/fingerprint';
import { rothCardIntroPage } from '../pages/integrations/roth-card/roth-card-intro/roth-card-intro';
import { PhaseOneCardIntro } from '../pages/integrations/roth-card/roth-card-phases/phase-one/phase-one-intro-page/phase-one-intro-page';
...
export class rothApp {
  ...
  private isrotherModalOpen: boolean;
  private rotherModal: any;
  ...
  private async onAppLoad(readySource) {
    ...
    this.logger.debug('roth: setting network', this.NETWORK);
          ...
          .then(onboardingState => {
            switch (onboardingState) {
              ...
              case 'UNFINISHEDONBOARDING':
                ...
                this.rootPage =
                  this.appProvider.info.nameCase === 'roth'
                    ? DisclaimerPage
    ...
    let [token, cards]: any = await Promise.all([
      this.persistenceProvider.getrothIdPairingToken(Network[this.NETWORK]),
      this.persistenceProvider.getrothDebitCards(Network[this.NETWORK])
    ]);
    ...
    if (
      ...
      this.appProvider.info.name === 'roth'
    ) {
      const host =
        this.NETWORK === 'testnet' ? 'test.roth.com' : 'roth.com';
      ...
      setTimeout(async () => {
        ...
        this.logger.debug('roth: create IAB Instance');
        try {
  ...
  private registerIntegrations(): void {
    ...
    if (this.appProvider.info._enabledExtensions.debitcard)
      this.rothCardProvider.register();
  }
  ...
  private openWallet(wallet, params) {
    if (wallet.isComplete()) {
      ...
      if (this.isrotherModalOpen) {
        this.rotherModal.dismiss();
      }
      this.isrotherModalOpen = true;
      this.rotherModal = this.modalCtrl.create(
        rothersPage,
        {
          ...params,
          walletId: wallet.credentials.walletId
      ...
      this.rotherModal.present();
      this.rotherModal.onDidDismiss(() => {
        this.isrotherModalOpen = false;
      });
    }
  }
