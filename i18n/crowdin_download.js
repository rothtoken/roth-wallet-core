  ...
  function (res) {
    ...
    res
      ...
      .on('end', function () {
        ...
        zip.extractAllTo('/tmp/roth', true);
        console.log('Done extracting ZIP file.');

        var files = fs.readdirSync('/tmp/roth/docs');
        ...
        var files = fs.readdirSync('/tmp/roth/po');

        for (var i in files) {
          var po_file = fs.readFileSync('/tmp/roth/po/' + files[i], 'utf8');
